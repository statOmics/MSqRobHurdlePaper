...
Perseus.AvsV <- read_xlsx(paste0(here(),"/datasets/PXD006675/41467_2017_1747_MOESM7_ESM.xlsx"), skip = 2)
...
```{r eval = FALSE}
file.proteinGroups <- file.proteinGroups <- paste0(wd,"/datasets/PXD006675/proteinGroups.txt")
...
```{r}
df.proteinGroups.HEART <- cbind(fData(proteinGroups.HEART1), Biobase::exprs(proteinGroups.HEART1))
...
sum(res[,1])
# 1496
sum(hurdle.AvsV$qchisq<0.01, na.rm = TRUE)
# 1496 gene identifiers were significant in the first stage
...
965+297
# 1262 gene identifiers are significant in at least one of both confirmation tests.
1496-1262
# For 234 gene identifiers, significance in the first stage could not clearly be attributed to either DD or DA

# Check:
234+965+297
# 1496

# 297 gene identifiers were both DA and DD in the second stage
...
796-297
# 499 proteins were DA, but not DD in the second stage

763-297
# 466 gene identifiers were DD, but not DA in the second stage
```
...
res.HEART.Hurdle[res.HEART.Hurdle$contrast == "regionLA-regionRA",][1:12,]
...
SERINC3_1 <- as.tibble(SERINC3_1) %>% mutate(Sequence = row.names(SERINC3_1))
...
SERINC3_2 <- as.tibble(SERINC3_2) %>% mutate(Sequence = row.names(SERINC3_2))
...
PNMA1_1 <- as.tibble(PNMA1_1) %>% mutate(Sequence = row.names(PNMA1_1))
...
PNMA1_2 <- as.tibble(PNMA1_2) %>% mutate(Sequence = row.names(PNMA1_2))
...
ACSM2A_1 <- as.tibble(ACSM2A_1) %>% mutate(Sequence = row.names(ACSM2A_1))
...
ACSM2A_2 <- as.tibble(ACSM2A_2) %>% mutate(Sequence = row.names(ACSM2A_2))
...
PDE7B_1 <- as.tibble(PDE7B_1) %>% mutate(Sequence = row.names(PDE7B_1))
...
PDE7B_2 <- as.tibble(PDE7B_2) %>% mutate(Sequence = row.names(PDE7B_2))
...
PNPLA7_1 <- as.tibble(PNPLA7_1) %>% mutate(Sequence = row.names(PNPLA7_1))
...
PNPLA7_2 <- as.tibble(PNPLA7_2) %>% mutate(Sequence = row.names(PNPLA7_2))
