...
all.proteins <- unique(unlist(lapply(list(res.CPTAC.Hurdle,CPTAC.Perseus.full,res.CPTAC.MSstats.full), "[", "protein")))
...
common.proteins <- unique(unlist(lapply(list(res.CPTAC.Hurdle,CPTAC.Perseus.full,res.CPTAC.MSstats.full), "[", "protein")))


indices <- which(table(c(unique(as.character(res.CPTAC.Hurdle$protein)), unique(as.character(CPTAC.Perseus.full$protein)), unique(as.character(res.CPTAC.MSstats.full$protein)))) == 3)

common.proteins <- names(table(c(unique(as.character(res.CPTAC.Hurdle$protein)), unique(as.character(CPTAC.Perseus.full$protein)), unique(as.character(res.CPTAC.MSstats.full$protein)))))[indices]

common.Base <- tibble(protein = rep(common.proteins, 3), 
                    UPS = grepl("ups", protein),
                    contrast = rep(c("condition6B-condition6A",
                  "condition6C-condition6B",
                  "condition6C-condition6A"), each = length(common.proteins)))
```

## 3. Make the FDR-FTP plots for the imputation-based methods (Supplementary Fig. 5)
...
colors <- c("#FF681E", "green", "blue", "black", "brown")
...
## 4. Make the FDR-FTP plots for count-based methods (Supplementary Fig. 6)
...
colors <- c("#FF681E", "#E15E9E", "darkgreen","darkgray")
...
## 5. Make the FDR-FTP plots for LFQ-based state-of-the-art methods (Supplementary Fig. 7)
...
colors <- c("#FF681E", "#1B2944", "#42B7BA", "magenta", "lightgreen", "peru") # darkblue: "#1B2944"  gray: "#50FF00" ? (dark)goldenrod?
...
## 6. Make the FDR-FTP plots for other state-of-the-art methods (Supplementary Fig. 8)
...
colors <- c("#FF681E", "black", "yellow3", "#808000", "kleur", "red2") # darkblue: "#1B2944"  gray: "#50FF00"
...
## 7. For the common set, make the FDR-FTP plots for LFQ-based state-of-the-art methods (Supplementary Fig. 9)
...
colors <- c("#FF681E", "#1B2944", "#42B7BA", "magenta", "green", "peru") # darkblue: "#1B2944"  gray: "#50FF00"
...
## 8. For the common set, make the FDR-FTP plots for other state-of-the-art methods (Supplementary Fig. 10)
...
## 9. Correlation between intensities and counts (Supplementary Fig. 11)
...
## 10. Plot example proteins for each of the overlapping regions in the Venn diagram (Supplementary Fig. 12 - 16)
...
## 11. Generate the data for the other Venn diagrams for the HEART dataset (Supplementary Fig. 17)
...
## 12. On the independence of the z-statistics under the null hypothesis  (Supplementary Fig. 18)
